...
pub fn select<T: FitnessFunction + Clone + Send>(
  ...
  Generation { individuals: generation[0..population_size].to_vec() }
}
...
pub fn compare<T: FitnessFunction + Clone>(a: &Individual<T>, b: &Individual<T>) -> Category {
  ...
  if is_duplicate(&a.network.genome, &b.network.genome) { return Category::Duplicate }
  ...
  for (gene_a, gene_b) in neurons {
    ...
    if gene_a.id != gene_b.id { return Category::Unique; }
  }
...
pub fn is_duplicate(a: &[Gene], b: &[Gene]) -> bool {
  // Observation: if the networks are different lengths, does this not immediately imply that they cannot be 
