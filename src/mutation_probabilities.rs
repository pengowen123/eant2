...
impl MutationProbabilities {
  ...
  pub(crate) fn generate(&self, rng: &mut ThreadRng) -> MutationType {
    ...
    let n = rng.gen_range(0..sum);

    let mut sum = 0;
    if n >= sum && n < sum + b {
      return MutationType::AddConnection;
    }
    sum += a;
    if n >= sum && n < sum + c {
      return MutationType::RemoveConnection;
    }
    sum += b;
    if n >= sum && n < sum + d {
      return MutationType::AddNode;
    } else {
      return MutationType::AddBias;
    }
  }
}
...
pub struct IllSpecifiedProbability;

impl TryFrom<MutationProbabilitiesFloat> for MutationProbabilities {
  type Error = IllSpecifiedProbability;

  fn try_from(p: MutationProbabilitiesFloat) -> Result<Self, IllSpecifiedProbability> {
    let p = p.0;
    let sum = p.0 + p.1 + p.2 + p.3;

    // all probabilities must sum above 0.0,
    // all probabilities must be non-negative.
    if sum == 0.0 || p.0 < 0. || p.1 < 0. || p.2 < 0. || p.3 < 0. {
      return Err(IllSpecifiedProbability);
    }
...
impl MutationProbabilitiesFloat {
  ...
  pub fn zeros() -> Self {
    MutationProbabilitiesFloat((0., 0., 0., 0.))
  }
}

impl MutationProbabilitiesFloat {
  /// The probability of adding a new connection between neurons.  Represented as the relative size of this `u16` to the others.
  pub fn add_connection(mut self, probability: f64) -> Self {
    self.0 .0 = probability;
    self
  }

  /// The probability of removing an existing connection between neurons.  Represented as the relative size of this `u16` to the others.
  pub fn remove_connection(mut self, probability: f64) -> Self {
    self.0 .1 = probability;
    self
  }

  /// The probability of adding a new neuron.  Represented as the relative size of this `u16` to the others.
  pub fn add_neuron(mut self, probability: f64) -> Self {
    self.0 .2 = probability;
    self
  }

  /// The probability of adding a new bias neuron.  Represented as the relative size of this `u16` to the others.
  pub fn add_bias(mut self, probability: f64) -> Self {
