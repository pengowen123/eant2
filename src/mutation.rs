...
fn add_connection<T: FitnessFunction + Clone>(
  ...
  if !valid_mutations.is_empty() {
    ...
    match mutation {
      ...
      1 => individual.add_forward(valid_forward[rng.gen_range(0..valid_forward.len())], index),
      // add recurrent neuron connection
      2 => individual.add_recurrent(
        valid_recurrent[rng.gen_range(0..valid_recurrent.len())],
        index,
      ),
      _ => unreachable!(),
    }
